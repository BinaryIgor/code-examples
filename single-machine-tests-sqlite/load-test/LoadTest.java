...
public class LoadTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf("Starting LoadTest on %s!%n", machinesString());
        System.out.println();
        System.out.printf("About to make %d requests with %d/s rate, on each machine%n", REQUESTS, REQUESTS_PER_SECOND);
    ...
    static List<Endpoint> endpoints() {
        ...
        // Same here; check out AccountController for naming pattern
        var existingNames = IntStream.range(0, 1000)
            .mapToObj(i -> "name-" + i)
            .toList();
        var accountsByNameEndpoint = accountsByName(existingNames);
        ...
        // 10% writes, 90% reads. Mostly by id - unique, but by name as well - collection, usually a few elements
        return List.of(
    ...
    static Endpoint accountsByName(List<String> existingNames) {
        var existingInstances = existingNames.stream().map(LoadTest::accountsByNameEndpointInstance).toList();
        return new Endpoint("GET: /accounts?name={name}", "GET",
            () -> {
                if (RANDOM.nextBoolean()) {
                    return randomChoice(existingInstances);
                }
                var nonExistingName = "name-" + UUID.randomUUID();
                return accountsByNameEndpointInstance(nonExistingName);
            });
    }

    static EndpointInstance accountsByNameEndpointInstance(String name) {
        return EndpointInstance.withoutBody(HOST + "/accounts?name=%s&secret=%s".formatted(name, SECRET_QUERY));
    }
    ...
    static void printStats(List<Long> sortedResults,
        ...
        System.out.printf("Executed requests on 1 machine: %d, with %d/s rate%n", allRequests, REQUESTS_PER_SECOND);
        ...
        System.out.println("Percentile 99.9: " + formattedSeconds(percentile999));
    }
    ...
    record TestProfileParams(int requests, int requestsPerSecond, int maxConcurrency) {
        ...
        static TestProfileParams ofRateFor30Seconds(int requestsPerSecond) {
            return new TestProfileParams(requestsPerSecond * 30, requestsPerSecond);
        }
    }
