  ...
  const html = `
    <div class="w-full h-full">
      <div class="w-full px-4 max-w-3xl absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        ...
        <sl-button class="my-8 hidden w-full" hx-target="#root">See Books</sl-button>
      </div>
      ${Web.scopedScript(`
        const categorySelect = document.querySelector("sl-select");
        const goToCategoryButton = document.querySelector("sl-button");
        let choosenCategory = ComponentsState.get("choosen-category");

        categorySelect.addEventListener("sl-change", e => {
          onChoosenCategory(e.target.value);
        });

        function onChoosenCategory(category) {
          goToCategoryButton.classList.remove("hidden");
          console.log("Chosen: ", category);
          choosenCategory = category;
          const booksCategoryUrl = "/books/" + choosenCategory;
          goToCategoryButton.setAttribute("hx-push-url", true);
          goToCategoryButton.setAttribute("hx-get", booksCategoryUrl);
          htmx.process(goToCategoryButton);
          ComponentsState.set("choosen-category", choosenCategory);
        }
        
        if (choosenCategory) {
           categorySelect.value = choosenCategory;
           onChoosenCategory(choosenCategory);
        }
        `)}
    </div>
    `;
