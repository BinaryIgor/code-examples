...
public class SqlCampaignRepository implements CampaignRepository {
    ...
    private static final String CAMPAIGN_TABLE = "campaign.campaign";
    ...
    public void save(Campaign campaign) {
        jdbcTemplate.update("""
                        INSERT INTO %s (id, name, budget_id, inventory_id, start_date, end_date)
                        VALUES (?, ?, ?, ?, ?, ?)
                        ON CONFLICT (id)
                        DO UPDATE SET
                          name = EXCLUDED.name,
    ...
    public Optional<Campaign> findById(UUID id) {
        var result = jdbcTemplate.query("SELECT * FROM %s WHERE id = ?".formatted(CAMPAIGN_TABLE),
