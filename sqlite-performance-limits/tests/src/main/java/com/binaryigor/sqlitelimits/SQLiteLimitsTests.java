...
public class SQLiteLimitsTests {
    ...
    // DELETE for default journal, WAL for Write Ahead Logging
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("About to run SQLite performance limits tests!");
        ...
        System.out.printf("Init queries executed, running %s test case!%n", TEST_CASE);
        ...
        printDelimiter();
        System.out.printf("Test case finished! It had queries: %s%n", testCase.queryGroups());
    ...
    static void runInitQueries(DataSource dataSource) {
        for (var q : SCHEMA.split(";")) {
            if (!q.isBlank()) {
                executeTimedQuery(dataSource, q);
            }
        }
        executeTimedQuery(dataSource, "PRAGMA journal_mode=" + JOURNAL_MODE);
        executeTimedQuery(dataSource, "PRAGMA busy_timeout=10000");
        if (!BEFORE_TESTS_QUERY.isEmpty()) {
            executeTimedQuery(dataSource, BEFORE_TESTS_QUERY);
        }
    }
    ...
    static TestCase testCase(DataSource dataSource) {
        ...
        var existingNames = existingNamesFromDb(dataSource, 3000);
        var existingEmails = existingEmailsFromDb(dataSource, 3000);

        return switch (TEST_CASE) {
            case BATCH_INSERTS -> batchInsertsTestCase(maxId);
            case WRITES_100 -> writes100(minId, maxId);
    ...
    static List<Long> minMaxIdsFromDb(DataSource dataSource) {
        return executeQuery(dataSource, "SELECT MIN(id), MAX(id) FROM account", r -> {
            if (r.next()) {
    ...
    static List<String> existingNamesFromDb(DataSource dataSource, int limit) {
        return executeQuery(dataSource, "SELECT DISTINCT name FROM account LIMIT " + limit, r -> {
    ...
    static List<String> existingEmailsFromDb(DataSource dataSource, int limit) {
        return executeQuery(dataSource, "SELECT DISTINCT email FROM account LIMIT " + limit, r -> {
    ...
    static TestCase batchInsertsTestCase(long lastId) {
        return new TestCase(
            ...
            batchInsertQueryGroup(envIntValueOrDefault("QUERIES_BATCH_SIZE", 100), new AtomicLong(lastId + 1)));
    }

    static QueryGroup batchInsertQueryGroup(int records, AtomicLong nextId) {
        return new QueryGroup("batch-insert", "account", () -> insertQuery(records, nextId));
    }

    static QueryGroup insertQueryGroup(AtomicLong nextId) {
        return new QueryGroup("insert", "account", () -> insertQuery(1, nextId));
    }

    static String insertQuery(int records, AtomicLong nextId) {
        return "INSERT INTO account (id, email, name, description, created_at, version) VALUES "
               + Stream.generate(() -> {
    ...
    static TestCase reads100TestCase(long minId, long maxId,
        ...
        return new TestCase(envQueriesToExecuteOrDefault(100_000),
    ...
    static QueryGroup selectByIdQueryGroup(long minId, long maxId) {
        return new QueryGroup("select-by-id", "account", () -> selectByIdQuery(minId, maxId));
    }

    static String selectByIdQuery(long minId, long maxId) {
        return "SELECT * FROM account WHERE id = %d".formatted(minId + RANDOM.nextLong(maxId));
    }

    static QueryGroup selectByNameQueryGroup(List<String> existingNames) {
        return new QueryGroup("select-by-name", "account", () -> selectByNameQuery(existingNames));
    }

    static String selectByNameQuery(List<String> names) {
        return "SELECT * FROM account WHERE name = '%s'".formatted(randomChoice(names));
    }

    static QueryGroup selectByEmailQueryGroup(List<String> existingEmails) {
        return new QueryGroup("select-by-email", "account", () -> selectByEmailQuery(existingEmails));
    }

    static String selectByEmailQuery(List<String> emails) {
        return "SELECT * FROM account WHERE email = '%s'".formatted(randomChoice(emails));
    }

    static TestCase writes100(long minId, long maxId) {
        return new TestCase(envQueriesToExecuteOrDefault(100_000),
            envQueriesMaxRateOrDefault(100),
            List.of(insertQueryGroup(new AtomicLong(maxId + 1)),
                updateByIdQueryGroup(minId, maxId),
                deleteByIdQueryGroup(minId, maxId)));
    }

    static TestCase updatesSelectsTestCase(long minId, long maxId,
                                           List<String> existingNames,
                                           int updatesRatio,
                                           int selectsRatio) {
        var queryGroups = Stream.of(
            Stream.generate(() -> updateByIdQueryGroup(minId, maxId))
                .limit(2L * updatesRatio),
        ...
        return new TestCase(envQueriesToExecuteOrDefault(100_000),
            envQueriesMaxRateOrDefault(100), queryGroups);
    }
    ...
    static QueryGroup updateByIdQueryGroup(long minId, long maxId) {
        return new QueryGroup("update-various-fields-by-id", "account", () -> updateByIdQuery(minId, maxId));
    }
    ...
    static String updateByIdQuery(long minId, long maxId) {
        ...
        return """
            ...
            SET name = '%s',
                ...
                version = %d
            WHERE id = %d
            """.formatted(newName, newEmail, newVersion, id);
    }

    static QueryGroup deleteByIdQueryGroup(long minId, long maxId) {
        return new QueryGroup("delete-by-id", "account", () -> deleteByIdQuery(minId, maxId));
    }

    static String deleteByIdQuery(long minId, long maxId) {
        return "DELETE FROM account WHERE id = %d".formatted(minId + RANDOM.nextLong(maxId));
    }
    ...
    static void printStats(List<QueryTestResult> sortedTestResults, Duration duration, int queriesRate) {
        ...
        System.out.printf("Wanted queries rate: %d/s%n", queriesRate);
        var realQueriesRate = sortedTestResults.size() / duration.getSeconds();
        System.out.printf("Real queries rate: %d/s%n", realQueriesRate);

        System.out.println();

        printStats(sortedTimeResults);
        ...
        groupedResults.forEach((qid, r) -> {
            System.out.println(qid + ": " + Math.round(r.size() * 100.0 / sortedTestResults.size()));
        });
        System.out.println();
        ...
        groupedResults.forEach((qid, r) -> {
            ...
            printStats(r.stream().map(QueryTestResult::time).toList());
        });
    }
