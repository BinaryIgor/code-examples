...
public class ProjectService implements ProjectClient {
    ...
    @Override
    public UUID idOfName(String name) {
        return projectRepository.ofName(name).map(Project::id)
            .orElseThrow(() -> new ProjectDoestNotExistException("Project of %s name doesn't exist".formatted(name)));
    }
