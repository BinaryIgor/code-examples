...
app="htmx-mpa-vs-react-spa-load-test"
tag="${TAG:-latest}"
...
export app=$app
export tag=$tag
export run_cmd="docker run -e HOST --name $app $tagged_image"

cd ..
envsubst '${app} ${tag}' < scripts/template_load_and_run_app.bash > load-test/dist/load_and_run_app.bash
envsubst '${app} ${run_cmd}' < scripts/template_run_app.bash > load-test/dist/run_app.bash
